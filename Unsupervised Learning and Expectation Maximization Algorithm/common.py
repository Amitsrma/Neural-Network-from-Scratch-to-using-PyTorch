...
def bic(X: np.ndarray, mixture: GaussianMixture,
    ...
    """
    def get_num_parameters(shape_of_matrix):
        prod=1
        for i in shape_of_matrix:
            prod *= i
        return prod
    parameters=0
    parameters+=get_num_parameters(mixture.mu.shape)
    parameters+=get_num_parameters(mixture.p.shape)
    parameters+=get_num_parameters(mixture.var.shape)
    BIC=log_likelihood-np.log(len(X))*1/2*(parameters-1)
    return BIC
